      ...
      <div className="container max-w-5xl grid gap-10">
        ...
        <section className="relative rounded-2xl border border-border/60 shadow-brand overflow-hidden">
          ...
          <div className="relative p-6 md:p-10 bg-card/65 backdrop-blur-xl">
            ...
            <div className="mt-6 grid sm:grid-cols-3 gap-3">
              {[
                { icon: <Gauge className="w-4 h-4" />, k: "Focus", v: "Embedded • Web • Mobile" },
                { icon: <Layers className="w-4 h-4" />, k: "Stack", v: "C++ · Python · Kotlin · Dart · Java · JS" },
                { icon: <Workflow className="w-4 h-4" />, k: "Approach", v: "Ship small • Iterate fast" },
              ].map((s) => (
                <div key={s.k} className="rounded-2xl border border-border bg-card/80 p-4 hover:shadow-brand-lg transition">
        ...
        <section className="grid lg:grid-cols-2 gap-4 items-stretch">
          <article className="rounded-2xl border border-border/70 bg-card p-6 md:p-8">
            <h2 className="text-2xl font-semibold mb-4">A short timeline</h2>
            <ol className="relative grid gap-9">
              <span className="pointer-events-none absolute left-3 top-0 bottom-0 w-px bg-[var(--border)]/60" />
              <li className="relative pl-8">
                <span className="absolute left-2 top-1.5 w-2.5 h-2.5 rounded-full bg-[var(--primary)]" />
                <div className="text-xs text-muted mb-1">2022 — Now</div>
                <p className="text-fg font-medium flex items-center gap-2">
                  Computer Engineering · UNNES
                  <span className="text-[10px] px-2 py-0.5 rounded-md border border-border/70 bg-bg text-muted">Ongoing</span>
                </p>
                <p className="text-sm text-muted">
                  Fokus kuat pada <span className="text-fg">IoT & embedded systems</span>: microcontrollers, perancangan PCB,
                  optimasi low-power, serta integrasi device-to-cloud.
                </p>
              </li>

              <li className="relative pl-8">
                <span className="absolute left-2 top-1.5 w-2.5 h-2.5 rounded-full bg-[var(--primary)]/80" />
                <div className="text-xs text-muted mb-1">2024 — Now</div>
                <p className="text-fg font-medium flex items-center gap-2">
                  Capstone Project: SICermat
                  <span className="text-[10px] px-2 py-0.5 rounded-md border border-border/70 bg-bg text-muted">Ongoing</span>
                </p>
                <p className="text-sm text-muted">
                  Wearable monitoring dengan ESP32-C3 (BMI270 & MAX30102), integrasi Edge Impulse ML, dan dashboard Flutter.
                </p>
              </li>

              <li className="relative pl-8">
                <span className="absolute left-2 top-1.5 w-2.5 h-2.5 rounded-full bg-[var(--primary)]/60" />
                <div className="text-xs text-muted mb-1">2024</div>
                <p className="text-fg font-medium">Rebuild Doswall App</p>
                <p className="text-sm text-muted">
                  Rebuild dari Dogoes (Kotlin) ke Flutter/Dart untuk cross-platform, dengan REST API dan geolokasi real-time.
                </p>
              </li>
            </ol>
          </article>
          ...
          <div className="grid gap-4 self-start">
            ...
            <article className="rounded-2xl border border-border/70 bg-card p-6 md:p-8">
              <h2 className="text-xl font-semibold mb-4">Skills at a glance</h2>
              <div className="grid sm:grid-cols-[1fr_1.4fr] gap-6 sm:gap-8">
                <div className="space-y-3">
                  <h3 className="text-sm text-muted">Languages</h3>
                  <div className="flex flex-wrap gap-2">
                    {["C++", "Python", "TypeScript", "JavaScript", "Kotlin", "Dart"].map((s) => (
                      <span key={s} className="text-sm px-3 py-1 rounded-xl border border-border/80 bg-card">{s}</span>
                    ))}
                  </div>
                </div>

                <div className="space-y-3 sm:pl-8 sm:border-l sm:border-[var(--border)]/60">
                  <h3 className="text-sm text-muted">Frameworks & Tools</h3>
                  <div className="flex flex-wrap gap-2">
        ...
        <section className="rounded-2xl border border-border/70 p-6 md:p-8 bg-card flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
